...
Shader "Toon/Toon (Tessellation)" {
    Properties
    {
        ...
        // Face SDF Shadow Properties
        [Toggle(_)] _UseSDFShadow ("Use SDF Face Shadow", Float) = 0
        _FaceShadowMap ("Face Shadow Map (SDF)", 2D) = "white" {}
        _FaceShadowMaskMap ("Face Fixed Shadow Map", 2D) = "black" {}
        _FaceShadowMaskStrength ("Face Fixed Shadow Strength", Range(0, 1)) = 1
        _FaceShadowOffset ("Face Shadow Offset", Range(-1, 1)) = 0
        _FaceShadowSoftness ("Face Shadow Softness", Range(0.0001, 0.5)) = 0.02
        _FaceForward ("Face Forward (Script)", Vector) = (0, 0, 1, 0)
        _FaceRight ("Face Right (Script)", Vector) = (1, 0, 0, 0)
        [Toggle(_)] _UseScriptVectors ("Use Script Vectors (else Object Space)", Float) = 0
        [Toggle(_)] _MirrorFaceShadowByLight ("Mirror Face Shadow By Light Side", Float) = 1
        _FaceShadowMirrorBlend ("Face Shadow Mirror Blend", Range(0.001, 0.5)) = 0.08
        _SDFFrontShadowSuppressStart ("SDF Front Shadow Suppress Start", Range(-1, 1)) = 0.65
        _SDFFrontShadowSuppressEnd ("SDF Front Shadow Suppress End", Range(-1, 1)) = 0.9
        [Enum(Off,0,FdotL,1,RdotL,2,FrontSuppress,3,ShadowMask,4,FixedShadow,5)] _SDFDebugMode ("SDF Debug Mode", Float) = 0
...
//            #pragma shader_feature _OCCLUSIONMAP
            ...
            #if (!defined(UNITY_COMPILER_DXC) && defined(UNITY_PLATFORM_VISIONOS)) || defined(SHADER_API_PS5)
