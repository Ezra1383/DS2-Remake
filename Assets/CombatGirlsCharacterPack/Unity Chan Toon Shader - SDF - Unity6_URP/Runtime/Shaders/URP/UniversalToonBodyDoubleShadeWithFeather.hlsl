...
void ToonShading(
    ...
    const float2 sdfUV, const float sdfShadowAttenuation, const float applySDFShadow,
    out float3 outShadeResult, out float outShadowMask, out float4 outSDFDebugValues
)
{
    ...
    outSDFDebugValues = float4(0.0, 0.0, 0.0, 0.0);
    if (_UseSDFShadow > 0.5 && applySDFShadow > 0.5)
    {
        Set_FinalShadowMask = ComputeFaceSDFShadowMask(sdfUV, lightDirection, sdfShadowAttenuation, outSDFDebugValues);
    }
...
void frag(VertexOutput i, out float4 finalRGBA : SV_Target0
    ...
    ToonShading(
        ...
        Set_UV0, shadowAttenuation, 1.0,
        Set_HighColor, Set_FinalShadowMask, sdfDebugValues);
    ...
    for (uint lightIndex = 0; lightIndex < min(URP_FP_DIRECTIONAL_LIGHTS_COUNT, MAX_VISIBLE_LIGHTS); lightIndex++)
    {
        ...
        int iLight = lightIndex;
        {
            ...
            ToonShading(
                ...
                specularBlendModeLerp, filterHighlightInForwardAdd, 
                Set_UV0, 1.0, 0.0,
                finalColor, unused, unusedSDFDebugValues);
    ...
    int iLight = lightIndex;
    {
        ...
        finalColor *= GetFaceSDFAdditionalLightMask(Set_UV0, lightDirection.xyz);
        finalColor = SATURATE_IF_SDR(finalColor);
    ...
    finalColor = ApplyFaceSDFDebugColor(finalColor, sdfDebugValues, Set_FinalShadowMask);
#endif
